...
#главная страница (route без параметра)
@app.route("/")
def index():
    candidates = get_all()
    result = '<br>'  #пустая строка
    for candidate in candidates:
        result += candidate['name'] + '<br>'
        result += candidate['position'] + '<br>'
        result += candidate['skills'] + '<br>'
        result += '<br>'   #чтобы была пустая строка между кандидатами

    return f'<pre> {result} </pre>'


#профиль пользователя /candidate/"номер pk"
...
#debug показывает место ошибки и также позволяет не перезапускать приложение